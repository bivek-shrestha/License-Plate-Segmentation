...
def preProcessingImage(inpImage):
    ...
    ret, thresholdFilter = cv2.threshold(
        gaussianFilter, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    morphFilter = cv2.morphologyEx(
        thresholdFilter, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)))
    return morphFilter
...
def cleanBorder(inpImg, radius):
    ...
    contours, hierarchy = cv2.findContours(
        inpImgCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for index in np.arange(len(contours)):
        ...
        for points in cnt:
            ...
            check1 = (rowContour >= 0 and rowContour < radius) or (
                rowContour >= imgRow - 1 - radius and rowContour < imgRow)
            check2 = (colContour >= 0 and colContour < radius) or (
                colContour >= imgCol - 1 - radius and colContour < imgCol)
...
def fillCharacters(inpImg):
    ...
    inpImgCopy = inpImg.copy()
    test = cv2.morphologyEx(inpImgCopy, cv2.MORPH_OPEN,
...
def characterSegment(inpImg, finalImg, mux):
    ...
    for cnts in contours:
        ...
        if w1 >= AverageWidth:
            ...
            cv2.rectangle(finalImg, (x1, y1),
                          (x1+w1, y1+h1), (255, 255, 255), 0)
...
# convert to binary
ret, thresholdImage = cv2.threshold(
    gaussianBlur, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
cv2.imshow("threshold", thresholdImage)

gradientImage = cv2.morphologyEx(
    thresholdImage, cv2.MORPH_GRADIENT, kernel=cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)))
...
# Drawing contours
contours, hierarchy = cv2.findContours(
    cannyEdge, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
...
for cnts in contours:
    ...
    if len(approx) == 4:
        ...
        if (ar >= 1.33 and ar <= 2.0) and (area >= minArea and area <= maxArea):
            imageIndex += 1
            rectImage = cv2.rectangle(
                inputImage, (x, y), (x+w, y+h), (0, 255, 0), 2)
...
croppedImage = cv2.imread("segmentTEST2.jpg")
...
inpImgCopyAreaBot = cleanBorderBottom[:cleanBorderBottom.shape[0] -
                                      15, 25:cleanBorderBottom.shape[1]-20]
